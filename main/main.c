...
{
    sht4x_init();
    ESP_LOGI(TAG, "SHT4x init done");
    while (1)
    {
        sht4x_take_measurement();
        float temperature = sht4x_get_temperature();
        float humidity = sht4x_get_humidity();
        ESP_LOGI(TAG, "Hum: %.2f Tmp: %.2f", temperature, humidity);
        uint16_t temperature_int = temperature * 100;
        uint16_t humidity_int = humidity * 100;
...
void app_main(void)
{
    ...
    if (tempSensorDataQueue == NULL)
        assert(0);
    
    // xTaskCreate(i2c_scan_task, "i2c_scan_task", 4096, NULL, 5, NULL);
